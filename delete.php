...
if(isset($data->id)){
    ...
    $check_post = "SELECT * FROM `productos` WHERE id=:post_id";
    ...
    if($check_post_stmt->rowCount() > 0){
        ...
        $delete_post = "DELETE FROM `productos` WHERE id=:post_id";
        $delete_post_stmt = $conn->prepare($delete_post);
        $delete_post_stmt->bindValue(':post_id', $post_id,PDO::PARAM_INT);
        
        if($delete_post_stmt->execute()){
            $msg['message'] = 'Se borro correctamente';
        }else{
            $msg['message'] = 'No se pudo Borrar';
        }
